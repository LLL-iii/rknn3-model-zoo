...
Result<std::unique_ptr<IRegex>> create_regex(const std::string& pattern) {
  // Patterns containing \p{...} (Unicode character-class shortcuts) need
  // the full Unicode property database that PCRE2 provides.  RE2's
  // unicode_groups.cc is built from an older Unicode version whose \p{L},
  // \p{N}, \p{P} and \p{S} ranges do not perfectly match Rust regex / ICU,
  // producing subtley different match boundaries for CJK/hiragana/maths
  // codepoints.  For ASCII-only or simple-code-point-range patterns RE2 is
  // still preferred because it is faster and lock-free.
  bool has_unicode_class = (pattern.find("\\p{") != std::string::npos);
  ...
  if (!has_unicode_class) {
    ...
    // RE2 compile failed for non-Unicode pattern — try fallback
  }
