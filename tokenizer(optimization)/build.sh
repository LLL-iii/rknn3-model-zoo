...
CXX_VER=$(${CXX_COMPILER} -dumpversion 2>/dev/null | cut -d. -f1)
if [ "${CXX_VER}" -lt 7 ]; then
    echo "============================================"
    echo "ERROR: C++17 compiler required by tokenizer."
    echo "Current compiler: ${CXX_COMPILER} (GCC ${CXX_VER})"
    echo "Required: GCC 7+ or Clang 7+"
    echo "============================================"
    exit 1
fi
echo "[OK] C++17 supported (GCC ${CXX_VER})"
...
# in from abseil / RE2 / sentencepiece / json / PCRE2 sub-projects.
# We only need Tokenizer.h + libtokenizer.a + tokenize_demo.
rm -rf "${INSTALL_DIR}/include/absl" \
       "${INSTALL_DIR}/include/pytorch" \
       "${INSTALL_DIR}/include/nlohmann" \
       "${INSTALL_DIR}/include/re2" \
