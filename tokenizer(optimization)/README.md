...
测试：单条 100,036 字符（UTF-8 151,949 字节）混合文本。纯编码耗时 = 长文本端到端中位数 − 空文本端到端中位数（消除模型加载），3 次取中位数。**Qwen3 代表值**（33,596 tokens）：

| 平台 | 纯编码 (ms) | 峰值 RSS (MB) | 吞吐 (kchars/s) |
|:-----|--------:|--------:|---------------:|
| x86_64 | 36.3 | — | 2,757 |
| Linux aarch64（RK3588） | 58.8 | 24.9 | 1,702 |
| Android arm64-v8a（RK3576） | 152.9 | 26.1 | 654 |

**分模型纯编码耗时**（毫秒，已启用 PCRE2 JIT，21 个模型全覆盖）：

| 模型 | x86_64 | RK3588 | RK3576 |
|:-----|------:|------:|------:|
| Qwen3 | 36.3 | 58.8 | 152.9 |
| Qwen2_5 | 33.9 | 92.2 | 338.5 |
| Qwen2_5_VL | 38.5 | 47.9 | 194.8 |
| Qwen2_5_Omni | 33.2 | 67.9 | 79.8 |
| Qwen3_VL | 38.7 | 72.9 | 400.2 |
| Qwen3_ASR | 32.1 | 61.7 | 219.4 |
| Qwen3_Embedding | 35.7 | 83.7 | 232.7 |
| Qwen3_Reranker | 34.2 | 53.2 | 376.2 |
| Qwen3_TTS | 41.9 | 78.4 | 241.7 |
| Qwen3_5 | 36.1 | 60.8 | 234.0 |
| Qwen3_VL_LoRA | 35.7 | 64.8 | 300.2 |
| GME-Qwen2-VL | 24.2 | 59.9 | 289.1 |
| InternVLM | 36.0 | 85.2 | 71.3 |
| HY_MT_1_5 | 33.7 | 75.9 | 203.8 |
| Janus_Pro | 42.2 | 101.4 | 308.5 |
| SmolVLM | 70.1 | 142.3 | 374.9 |
| SmolVLM2 | 66.1 | 160.2 | 330.4 |
| glm_edge | 39.0 | 66.7 | 189.9 |
| MiniCPM_V_4 | 35.4 | 104.9 | 503.8 |
| FastVLM | 52.5 | 159.6 | 485.7 |
| gemma4 | 33.7 | 104.9 | 175.3 |
| **平均** | **39.5** | **85.9** | **271.6** |

> **JIT 优化**：瓶颈为 Sequence `[Split, ByteLevel]` 预分词的正则匹配（PCRE2 每 match 固定开销 ~50-100µs，混合文本切 4 万+ piece）。通过 `PCRE2_SUPPORT_JIT` 强制开启 + `pcre2_jit_match`，单条 100k 编码从 0.74/2.19/3.82 s 降到 **39.5 / 85.9 / 271.6 ms**（-94%~-95%），Qwen3 提速 27~50 倍，达到/超过 llama.cpp 水平。Cygwin 因 sljit 运行时 segfault 禁用 JIT，Linux/Android 启用。
