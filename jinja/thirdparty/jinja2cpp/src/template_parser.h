...
{
    template<typename CharT>
    auto GetName() const -> std::basic_string<CharT>
    {
...
private:
            ...
            case RM_ExprEnd:
                if (m_currentBlockInfo.type == TextBlockType::RawText)
                {
                    ...
                    return MakeParseError(ErrorCode::UnexpectedExprEnd, MakeToken(Token::ExprEnd, { matchStart, matchStart + 2 }));
                         ...
                         || IsInsideQuote(m_currentBlockInfo.range.startOffset, match.position()))
                    break;
    ...
    // RKNN3: check whether [start, pos) ends inside a quoted string (for '}}'/'%}' inside string literals)
    bool IsInsideQuote(size_t start, size_t pos) const
    {
        char quote = 0;
        bool escaped = false;
        size_t n = m_template->size();
        for (size_t i = start; i < pos && i < n; ++i)
        {
            char c = (*m_template)[i];
    ...
    size_t StripBlockRight(TextBlockInfo& /* currentBlockInfo */, size_t position, bool trimBlocks)
    {
        ...
        if ((m_currentBlockInfo.type != TextBlockType::RawText) && position != 0)
        {
            ...
            {
                doTrim = true;
                manualTrim = true; // `-%}`：Python manual trim，删除标签后所有空白（含多行缩进）
            }
            else if (ctrlChar == '+')
                doTrim = false;
        }
        ...
        if (doTrim)
        {
            ...
            for (; newPos < m_template->size(); ++newPos)
            {
                ...
                if (!std::isspace(ch, locale))
                    break;
                // trim_blocks（配置项）：只删到行尾；manual trim（-%}）：删所有空白
                if (ch == '\n' && !manualTrim)
                {
                    ++newPos;
                    break;
    ...
    nonstd::expected<R, ParseError> InvokeParser(const TextBlockInfo& block, Args&&... args)
    {
        ...
        tokenizer.begin();
        auto adjust = range.startOffset;
        Lexer lexer(
          [&tokenizer, adjust]() mutable {
...
KeywordsInfo ParserTraitsBase<T>::s_keywordsInfo[43] = {
    ...
    { UNIVERSAL_STR("generation"), Keyword::Generation },
    { UNIVERSAL_STR("endgeneration"), Keyword::EndGeneration },
};
