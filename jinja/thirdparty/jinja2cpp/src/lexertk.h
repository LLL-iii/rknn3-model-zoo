...
namespace lexertk
{
    ...
    struct CharRange
    {
        ...
        auto length() const -> ptrdiff_t {return end - start;}
        auto offset(CharT* from) const -> ptrdiff_t {return start - from;}
        auto operator[] (size_t idx) const -> CharT {return start[idx];}
    };
   namespace details
   {
#if 0
      ...
      struct lexer_traits
      {
          static auto get_locale() -> std::locale&
          {
