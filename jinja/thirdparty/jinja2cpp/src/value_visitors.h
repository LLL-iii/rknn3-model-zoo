...
struct RecursiveUnwrapper
{
    ...
    template<typename T>
    static auto UnwrapRecursive(const T& arg) -> const T&
    {
        return arg; // std::forward<T>(arg);
    }

    template<typename T>
    static auto UnwrapRecursive(const RecursiveWrapper<T>& arg) -> const T&
    {
        return arg.GetValue();
    }
...
//    }
    ...
    auto operator()(const Args& ... args) const
        -> decltype((*std::declval<V*>())(UnwrapRecursive(args)...))
    {
...
auto ApplyUnwrapped(const InternalValueData& val, Fn&& fn) -> decltype(fn(val))
{
...
// RKNN3: C++11 降级——泛型 lambda 改为仿函数
template<typename V, typename ... Args>
class ApplyVisitor
{
public:
    explicit ApplyVisitor(Args&& ... args)
        : m_v(V(std::forward<Args>(args)...))
    {
    }
    template<typename T>
    auto operator()(T& val)
        -> decltype(nonstd::visit(detail::RecursiveUnwrapper<V>(&std::declval<V&>()), val))
    {
        return nonstd::visit(detail::RecursiveUnwrapper<V>(&m_v), val);
    }
private:
    V m_v;
};
...
{
public:
    Apply2Inner(V* v, T1& val1)
        : m_v(v), m_val1(val1)
    {
    }
    template<typename T2>
    auto operator()(T2& val2)
        -> decltype(nonstd::visit(detail::RecursiveUnwrapper<V>(std::declval<V*>()), std::declval<T1&>(), val2))
    {
        return nonstd::visit(detail::RecursiveUnwrapper<V>(m_v), m_val1, val2);
    }
private:
    V* m_v;
    T1& m_val1;
...
{
public:
    Apply2Visitor(const InternalValue& val2, Args&& ... args)
        : val2_(val2), m_v(V(std::forward<Args>(args)...))
    {
    }
    template<typename T>
    auto operator()(T& val1)
        -> decltype(detail::ApplyUnwrapped(std::declval<const InternalValueData&>(), detail::Apply2Inner<V, T>(std::declval<V*>(), val1)))
    {
        return detail::ApplyUnwrapped(val2_.GetData(), detail::Apply2Inner<V, T>(&m_v, val1));
    }
private:
    const InternalValue& val2_;
    V m_v;
};
} // namespace detail
...
auto Apply(const InternalValue& val, Args&& ... args)
    -> decltype(detail::ApplyUnwrapped(val.GetData(), detail::ApplyVisitor<V, Args...>(std::forward<Args>(args)...)))
{
    return detail::ApplyUnwrapped(val.GetData(), detail::ApplyVisitor<V, Args...>(std::forward<Args>(args)...));
}

template<typename V, typename ... Args>
auto Apply2(const InternalValue& val1, const InternalValue& val2, Args&& ... args)
    -> decltype(detail::ApplyUnwrapped(val1.GetData(), detail::Apply2Visitor<V, Args...>(val2, std::forward<Args>(args)...)))
{
    return detail::ApplyUnwrapped(val1.GetData(), detail::Apply2Visitor<V, Args...>(val2, std::forward<Args>(args)...));
}
...
struct ValueRendererBase
{
    ...
    void operator()(const Slice&) const {}  // RKNN3: slice index renders as empty
    ...
    auto GetOs() const -> std::back_insert_iterator<std::basic_string<CharT>> { return std::back_inserter(*m_os); }
...
struct BinaryMathOperation : BaseVisitor<>
{
    ...
    typename std::enable_if<!std::is_same<CharT1, CharT2>::value, ResultType>::type operator() (const std::basic_string<CharT1>& left, const std::basic_string<CharT2>& right) const
    {
    ...
    template<typename CharT1, typename CharT2>
    typename std::enable_if<!std::is_same<CharT1, CharT2>::value, ResultType>::type operator() (const nonstd::basic_string_view<CharT1>& left, const std::basic_string<CharT2>& right) const
    {
        auto rightStr = ConvertString<std::basic_string<CharT1>>(right);
    ...
    template<typename CharT1, typename CharT2>
    typename std::enable_if<!std::is_same<CharT1, CharT2>::value, ResultType>::type operator() (const std::basic_string<CharT1>& left, const nonstd::basic_string_view<CharT2>& right) const
    {
        auto rightStr = ConvertString<std::basic_string<CharT1>>(right);
        return ProcessStrings(nonstd::basic_string_view<CharT1>(left), nonstd::basic_string_view<CharT1>(rightStr));
    }
    ...
    template<typename CharT1, typename CharT2>
    typename std::enable_if<!std::is_same<CharT1, CharT2>::value, ResultType>::type operator() (const nonstd::basic_string_view<CharT1>& left, const nonstd::basic_string_view<CharT2>& right) const
    {
        auto rightStr = ConvertString<std::basic_string<CharT1>>(right);
        return ProcessStrings(left, nonstd::basic_string_view<CharT1>(rightStr));
    }
    ...
    ResultType operator() (const ListAdapter& left, int64_t right) const
    {
        ...
        if (right >= 0 && m_oper == jinja2::BinaryExpression::Mul)
        {
            ...
            result = ListAdapter::CreateAdapter(static_cast<size_t>(listSize),
                                                [size, vals](size_t idx) { return vals[idx % size]; });
        }
...
struct StringJoiner : BaseVisitor<TargetString>
{
    ...
    template<typename CharT1, typename CharT2>
    typename std::enable_if<!std::is_same<CharT1, CharT2>::value, TargetString>::type operator() (const std::basic_string<CharT1>& left, const std::basic_string<CharT2>& right) const
    {
    ...
    template<typename CharT1, typename CharT2>
    typename std::enable_if<!std::is_same<CharT1, CharT2>::value, TargetString>::type operator() (std::basic_string<CharT1> left, const nonstd::basic_string_view<CharT2>& right) const
    {
...
struct SameStringGetter : public visitors::BaseVisitor<nonstd::expected<void, std::basic_string<CharT>>>
{
    ...
    using OtherString = typename std::conditional<std::is_same<CharT, char>::value, std::wstring, std::string>;
    using ResultStringView = nonstd::basic_string_view<CharT>;
    using OtherStringView = typename std::conditional<std::is_same<CharT, char>::value, nonstd::wstring_view, nonstd::string_view>;
...
auto ApplyStringConverter(const InternalValue& str, Fn&& fn)
    -> decltype(Apply<Cvt<Fn>>(str, std::forward<Fn>(fn)))
{
...
auto GetAsSameString(const std::basic_string<CharT>&, const InternalValue& val)
    -> nonstd::optional<std::basic_string<CharT>>
{
