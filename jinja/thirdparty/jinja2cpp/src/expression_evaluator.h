...
// RKNN3: slice index expression [start:stop:step]
class SliceExpression : public Expression
{
public:
    SliceExpression(ExpressionEvaluatorPtr<> start, ExpressionEvaluatorPtr<> stop, ExpressionEvaluatorPtr<> step)
    ...
    InternalValue Evaluate(RenderContext& values) override
    {
        ...
        if (m_start)
        {
            auto v = m_start->Evaluate(values);
            if (auto* p = GetIf<int64_t>(&v))
                sl.start = *p;
        }
        if (m_stop)
        {
            auto v = m_stop->Evaluate(values);
            if (auto* p = GetIf<int64_t>(&v))
                sl.stop = *p;
        }
        if (m_step)
        {
            auto v = m_step->Evaluate(values);
            if (auto* p = GetIf<int64_t>(&v))
                sl.step = *p;
        }
        return InternalValue(sl);
    }
    ...
    bool IsEqual(const IComparable& other) const override
    {
        ...
        if (!val)
            return false;
        return m_start == val->m_start && m_stop == val->m_stop && m_step == val->m_step;
    }
...
public:
    ...
    IsExpression(ExpressionEvaluatorPtr<> value, const std::string& tester, CallParamsInfo params, bool negate = false);
...
private:
    ...
    TesterPtr m_tester;
    bool m_negate = false;  // RKNN3: support 'is not <test>'
};
...
public:
    ...
    auto GetValueRef() const -> const ExpressionEvaluatorPtr<>& {return m_valueRef;}
    auto GetParams() const -> const CallParamsInfo& {return m_params;}
...
private:
    ...
    InternalValue CallGlobalNamespace(RenderContext &values);
