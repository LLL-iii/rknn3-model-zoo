...
// Attribute 被 map(attribute=...) 内部转成 attr filter 使用，必须保留
...
// RKNN3: items filter（dict -> list of (key,value) KeyValuePair，供 `for k,v in x|items` 解构）
class Items : public FilterBase
{
public:
    Items(FilterParams params);
    InternalValue Filter(const InternalValue& baseVal, RenderContext& context) override;
    bool IsEqual(const IComparable& other) const override
    {
        auto* value = dynamic_cast<const Items*>(&other);
        if (!value)
            return false;
        return m_args == value->m_args;
    }
};

// RKNN3: safe filter（no-op；Jinja2Cpp 无自动 HTML 转义，原样返回）
class SafeFilter : public FilterBase
{
public:
    SafeFilter(FilterParams params);
    InternalValue Filter(const InternalValue& baseVal, RenderContext& context) override;
    bool IsEqual(const IComparable& other) const override
    {
        auto* value = dynamic_cast<const SafeFilter*>(&other);
        if (!value)
            return false;
        return m_args == value->m_args;
    }
...
// RKNN3: string filter（转字符串）
class StringFilter : public FilterBase
{
public:
    StringFilter(FilterParams params);
    InternalValue Filter(const InternalValue& baseVal, RenderContext& context) override;
    bool IsEqual(const IComparable& other) const override
    {
        auto* value = dynamic_cast<const StringFilter*>(&other);
        if (!value)
            return false;
        return m_args == value->m_args;
    }
