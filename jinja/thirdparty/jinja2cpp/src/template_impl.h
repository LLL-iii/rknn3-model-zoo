...
struct TemplateLoader<char>
{
    static auto Load(const std::string& fileName, TemplateEnv* env)
        -> decltype(env->LoadTemplate(fileName))
    {
...
{
    static auto Load(const std::string& fileName, TemplateEnv* env)
        -> decltype(env->LoadTemplateW(fileName))
    {
...
public:
    ...
    boost::optional<ErrorInfoTpl<CharT>> Render(std::basic_string<CharT>& os, const ValuesMap& params)
    {
        ...
        try
        {
            ...
            auto writer = GenericStreamWriter<CharT>(os);
            OutStream outStream([writer]() mutable -> OutStream::StreamWriter* {return &writer;});
...
private:
    ...
    public:
        ...
        OutStream GetStreamOnString(TargetString& str) override
        {
            ...
            str = string_t();
            auto writer = StringStreamWriter<CharT>(&nonstd::get<string_t>(str));
            return OutStream([writer]() mutable -> OutStream::StreamWriter* { return &writer; });
        }
...
private:
    ...
    mutable boost::any m_metadataJson;
