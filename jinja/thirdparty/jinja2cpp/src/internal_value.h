...
#include "string_literals_compat.h"
#endif
...
template<typename T>
auto MakeWrapped(T&& val) -> RecursiveWrapper<typename std::decay<T>::type>
{
    return RecursiveWrapper<typename std::decay<T>::type>(std::forward<T>(val));
}
...
// RKNN3: slice index for [start:stop:step]; nullopt means "default"
struct Slice
{
...
{
    template<typename V>
    static auto Get(V&& val) -> decltype(nonstd::get<T>(std::forward<V>(val).GetData()))
    {
        return nonstd::get<T>(std::forward<V>(val).GetData());
    }

    static auto GetPtr(const InternalValue* val) -> const T*;

    static auto GetPtr(InternalValue* val) -> T*;

    template<typename V>
    static auto GetPtr(V* val, typename std::enable_if<!std::is_same<V, InternalValue>::value>::type* = nullptr) -> decltype(nonstd::get_if<T>(val))
    {
...
{
    template<typename V>
    static auto Get(V&& val) -> decltype(nonstd::get<RecursiveWrapper<T>>(std::forward<V>(val)).GetValue())
    {
    ...
    template<typename V>
    static auto GetPtr(V* val, typename std::enable_if<!std::is_same<V, InternalValue>::value>::type* = nullptr) -> decltype(nonstd::get_if<RecursiveWrapper<T>>(val))
    {
...
public:
    ...
    template<typename T>
    InternalValue(T&& val, typename std::enable_if<!std::is_same<typename std::decay<T>::type, InternalValue>::value && !std::is_arithmetic<typename std::decay<T>::type>::value>::type* = nullptr)
        : m_data(InternalValueData(std::forward<T>(val)))
    {
    }

    auto GetData() const -> const InternalValueData& {return m_data;}
    auto GetData() -> InternalValueData& {return m_data;}

    auto GetParentData() -> InternalValueData& {return m_parentData;}
    auto GetParentData() const -> const InternalValueData& {return m_parentData;}
...
inline auto ValueGetter<T, V>::GetPtr(const InternalValue* val) -> const T*
{
    return nonstd::get_if<T>(&val->GetData());
}

template<typename T, bool V>
inline auto ValueGetter<T, V>::GetPtr(InternalValue* val) -> T*
{
    return nonstd::get_if<T>(&val->GetData());
}

template<typename T>
inline auto ValueGetter<T, true>::GetPtr(const InternalValue* val) -> const T*
{
...
template<typename T>
inline auto ValueGetter<T, true>::GetPtr(InternalValue* val) -> T*
{
    auto ref = nonstd::get_if<RecursiveWrapper<T>>(&val->GetData());
    return !ref ? nullptr : &ref->GetValue();
}
...
auto Get(V&& val) -> decltype(ValueGetter<T, IsRecursive<T>::value>::Get(std::forward<V>(val).GetData()))
{
    return ValueGetter<T, IsRecursive<T>::value>::Get(std::forward<V>(val).GetData());
}

template<typename T, typename V>
auto GetIf(V* val) -> decltype(ValueGetter<T, IsRecursive<T>::value>::GetPtr(val))
{
...
public:
    ...
    auto GetType() const -> Callable::Type
    {
    ...
    auto GetCallable() const -> const CallableHolder&
    {
        return m_callable;
    }

    auto GetExpressionCallable() const -> const ExpressionCallable&
    {
        return nonstd::get<ExpressionCallable>(m_callable);
    }

    auto GetStatementCallable() const -> const StatementCallable&
    {
...
auto MakeDynamicProperty(Fn&& fn) -> MapAdapter
{
...
auto sv_to_string(const nonstd::basic_string_view<CharT>& sv) -> std::basic_string<CharT>
{
