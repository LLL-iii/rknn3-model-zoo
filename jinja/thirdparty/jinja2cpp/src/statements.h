...
protected:
    ...
    const std::vector<std::string> m_fields;  // RKNN3: protected for member-assignment in SetLineStatement
};
...
public:
    ...
    SetLineStatement(std::vector<std::string> fields, ExpressionEvaluatorPtr<> expr,
        ...
        : SetStatement(std::move(fields)), m_expr(std::move(expr)), m_member(std::move(member))
    {
    }
    ...
    bool IsEqual(const IComparable& other) const override
    {
        ...
        if (m_expr != val->m_expr)
            return false;
        if (m_member != val->m_member)
            return false;
        return true;
    }
private:
    const ExpressionEvaluatorPtr<> m_expr;
    const std::string m_member;  // RKNN3: non-empty => 'set ns.attr = value' member assignment
};
