...
struct FindInMap
{
    ...
    auto operator()(Map& map) const -> decltype(map.find(val))
    {
        auto p = map.find(val);
        if (p != map.end())
            found = true;
...
public:
    ...
    auto FindValue(const std::string& val, bool& found) const -> typename InternalValueMap::const_iterator
    {
        FindInMap finder{val, found};
    ...
    auto GetCurrentScope() const -> const InternalValueMap&
    {
        return *m_currentScope;
    }

    auto GetCurrentScope() -> InternalValueMap&
    {
        return *m_currentScope;
    }
    auto GetGlobalScope() -> InternalValueMap&
    {
        return m_scopes.front();
    }
    auto GetRendererCallback() -> IRendererCallback*
    {
