...
struct TemplateFunctions<char>
{
    ...
    static auto LoadFile(const std::string& fileName, const IFilesystemHandler* fs) -> decltype(fs->OpenStream(fileName)) { return fs->OpenStream(fileName); }
};
...
struct TemplateFunctions<wchar_t>
{
    ...
    static auto LoadFile(const std::string& fileName, const IFilesystemHandler* fs) -> decltype(fs->OpenWStream(fileName)) { return fs->OpenWStream(fileName); }
};

template<typename CharT, typename T, typename Cache>
auto TemplateEnv::LoadTemplateImpl(TemplateEnv* env, std::string fileName, const T& filesystemHandlers, Cache& cache) -> TemplateEnvResult<CharT>
{
