...
struct CharsAppender
{
    Str& result;
    explicit CharsAppender(Str& r) : result(r) {}
    template<typename... Chs>
    void operator()(Chs... chs) const
    {
        using value_type = typename Str::value_type;
        int expand[] = {0, ((void)result.push_back(static_cast<value_type>(chs)), 0)...};
        (void)expand;
    }
};
...
struct TrimStringVisitor
{
    ...
    TargetString operator()(nonstd::basic_string_view<CharT> strView) const
    {
...
template<typename IsAlphaT, bool ToUpper>
struct CaseConverter
{
    IsAlphaT& isAlpha;
    explicit CaseConverter(IsAlphaT& ia) : isAlpha(ia) {}
    template<typename CharT, typename AppendFn>
    void operator()(CharT ch, AppendFn&& fn) const
    {
        if (isAlpha(ch))
            fn(ToUpper ? std::toupper(ch, std::locale()) : std::tolower(ch, std::locale()));
        else
            fn(ch);
    }
};

template<typename IsAlphaT>
struct CapitalConverter
{
    IsAlphaT& isAlpha;
    mutable bool isFirstChar;
    explicit CapitalConverter(IsAlphaT& ia) : isAlpha(ia), isFirstChar(true) {}
    template<typename CharT, typename AppendFn>
    void operator()(CharT ch, AppendFn&& fn) const
    {
        if (isAlpha(ch))
...
struct StringEncoder : public visitors::BaseVisitor<TargetString>
{
    ...
    TargetString operator() (const std::basic_string<CharT>& str) const
    {
        ...
        for (auto& ch : str)
        {
            static_cast<const D*>(this)->EncodeChar(ch, CharsAppender<decltype(result)>(result));
        }
...
struct UrlStringEncoder : public StringEncoder<UrlStringEncoder>
{
    ...
    {
        static auto Cast(Ch ch) -> uint8_t {return static_cast<uint8_t>(ch);}
    };

    template<typename Ch>
    struct ToUnsigned<Ch, 2>
    {
        static auto Cast(Ch ch) -> uint16_t {return static_cast<uint16_t>(ch);}
    };

    template<typename Ch>
    struct ToUnsigned<Ch, 4>
    {
        static auto Cast(Ch ch) -> uint32_t {return static_cast<uint32_t>(ch);}
    };

    template<typename Ch>
    auto AsUnsigned(Ch ch) const -> uint32_t
    {
...
InternalValue StringConverter::Filter(const InternalValue& baseVal, RenderContext& context)
{
    ...
    {
    case TrimMode:
        result = ApplyStringConverter(baseVal, TrimStringVisitor());
        break;
...
#endif
    case UpperMode:
        result = ApplyStringConverter<GenericStringEncoder>(baseVal, CaseConverter<decltype(isAlpha), true>(isAlpha));
        break;
    case LowerMode:
        result = ApplyStringConverter<GenericStringEncoder>(baseVal, CaseConverter<decltype(isAlpha), false>(isAlpha));
        break;
#ifndef JINJA2CPP_CHAT_ONLY
    case ReplaceMode:
        result = ApplyStringConverter(baseVal, [this, &context](auto srcStr) -> TargetString {
            typename std::decay<decltype(srcStr)>::type emptyStrView;
...
#endif
    case CapitalMode:
        result = ApplyStringConverter<GenericStringEncoder>(baseVal, CapitalConverter<decltype(isAlpha)>(isAlpha));
        break;
#ifndef JINJA2CPP_CHAT_ONLY
    case EscapeHtmlMode:
