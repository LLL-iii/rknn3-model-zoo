...
auto ReplaceErrorIfPossible(T& result, const Token& pivotTok, ErrorCode newError)
    -> decltype(MakeParseError(newError, pivotTok))
{
...
ExpressionParser::ParseResult<ExpressionEvaluatorPtr<Expression>> ExpressionParser::ParseLogicalCompare(LexScanner& lexer)
{
    ...
    default:
        ...
        case Keyword::Is:
        {
            ...
            else if (lexer.GetAsKeyword(nextTok) == Keyword::None)
                name = "none";
            else if (lexer.GetAsKeyword(nextTok) == Keyword::True)
                name = "true";
            else if (lexer.GetAsKeyword(nextTok) == Keyword::False)
                name = "false";
            else
            ...
            return std::make_shared<IsExpression>(*left, std::move(name), std::move(*params), negate);
        }
        case Keyword::LogicalNot:
        {
            // RKNN3 patch: support 'not in' (e.g. 'key not in standard_keys')
            Token inTok = lexer.NextToken();
            if (lexer.GetAsKeyword(inTok) == Keyword::In)
            {
...
ExpressionParser::ParseResult<ExpressionEvaluatorPtr<Expression>> ExpressionParser::ParseValueExpression(LexScanner& lexer)
{
    ...
    case Token::String:
    {
        // RKNN3: adjacent string literal concatenation ("a" "b" -> "ab")
        std::string str = AsString(tok.value);
        while (lexer.PeekNextToken() == Token::String)
        {
            auto nt = lexer.NextToken();
            str += AsString(nt.value);
        }
        // RKNN3: 不能直接 return，否则后续下标/调用（如 "abc"[0]、"x".upper()）无法解析
        valueRef = std::make_shared<ConstantExpression>(InternalValue(str));
        break;
    }
    case Token::True:
    ...
    if (valueRef)
    {
        // RKNN3: loop to support chained attr/subscript/call (e.g. part.split('x')[0])
        while (true)
        {
...
ExpressionParser::ParseResult<ExpressionEvaluatorPtr<Expression>> ExpressionParser::ParseSubscript(LexScanner& lexer, ExpressionEvaluatorPtr<Expression> valueRef)
{
    ...
    for (Token tok = lexer.NextToken(); tok.type == '.' || tok.type == '['; tok = lexer.NextToken())
    {
        ...
        {
            // RKNN3: support slice [start:stop(:step)]
            ExpressionEvaluatorPtr<Expression> startExpr;
            ...
            {
                auto start = ParseFullExpression(lexer);
                if (!start)
                    return start.get_unexpected();
                startExpr = *start;
            }
            ...
            else
            {
                ...
                ExpressionEvaluatorPtr<Expression> stopExpr;
                if (lexer.PeekNextToken() != ':' && lexer.PeekNextToken() != ']')
                {
                    auto stop = ParseFullExpression(lexer);
                    if (!stop)
                        return stop.get_unexpected();
                    stopExpr = *stop;
                }
                ExpressionEvaluatorPtr<Expression> stepExpr;
                if (lexer.PeekNextToken() == ':')
                {
                    lexer.NextToken();
                    if (lexer.PeekNextToken() != ']')
                    {
                        auto step = ParseFullExpression(lexer);
                        if (!step)
                            return step.get_unexpected();
                        stepExpr = *step;
                    }
                }
                indexExpr = std::make_shared<SliceExpression>(startExpr, stopExpr, stepExpr);
            }
