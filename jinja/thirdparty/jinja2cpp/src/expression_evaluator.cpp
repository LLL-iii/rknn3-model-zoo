...
InternalValue BinaryExpression::Evaluate(RenderContext& context)
{
    ...
    {
        // Python 语义：a and b → 左为假返回左操作数，否则返回右操作数
        bool left = ConvertToBool(leftVal);
        result = left ? rightVal : leftVal;
        break;
    }
    case jinja2::BinaryExpression::LogicalOr:
    {
        // Python 语义：a or b → 左为真返回左操作数，否则返回右操作数
        bool left = ConvertToBool(leftVal);
        result = left ? leftVal : rightVal;
        break;
    }
...
IsExpression::IsExpression(ExpressionEvaluatorPtr<> value, const std::string& tester, CallParamsInfo params, bool negate)
    : m_value(value)
    , m_negate(negate)
{
...
{
    bool result = m_tester->Test(m_value->Evaluate(context), context);
...
InternalValue CallExpression::Evaluate(RenderContext& values)
{
    ...
    case NamespaceFn:
        return CallGlobalNamespace(values);
    default:
...
InternalValue CallExpression::CallGlobalNamespace(RenderContext& values)
{
    // RKNN3: namespace(**kwargs) -> mutable map with namespace semantics
    InternalValueMap m;
    for (auto& np : m_params.kwParams)
        m[np.first] = np.second->Evaluate(values);
    return InternalValue(CreateMapAdapter(std::move(m)));
}

void SetupGlobals(InternalValueMap& globalParams)
{
    globalParams["range"] = InternalValue(static_cast<int64_t>(RangeFn));
    globalParams["namespace"] = InternalValue(static_cast<int64_t>(NamespaceFn));
    // RKNN3: raise_exception(msg) 对齐 transformers 全局函数——抛异常使渲染失败。
    // 异常沿 Render 链传播，被封装层 ChatTemplate::Render 的 catch 捕获并返回 false。
    globalParams["raise_exception"] = InternalValue(Callable(Callable::GlobalFunc,
        [](const CallParams& params, RenderContext&) -> InternalValue {
            std::string msg = "raise_exception called";
            if (!params.posParams.empty())
                msg = AsString(params.posParams[0]);
...
{
    static auto Get(const InternalValue& val) -> InternalValue { return val; }
};

template<>
struct ParsedArgumentDefaultValGetter<ParsedArgumentsInfo>
{
    static auto Get(const InternalValue& val) -> std::shared_ptr<ConstantExpression> { return std::make_shared<ConstantExpression>(val); }
};
