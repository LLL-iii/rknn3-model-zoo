...
StatementsParser::ParseResult StatementsParser::Parse(LexScanner& lexer, StatementInfoList& statementsInfo)
{
    ...
    case Keyword::EndGeneration:
        // RKNN3: HF `{% generation %}` tag — ignore the tag, render body as-is
        break;
    case Keyword::With:
...
auto MakeParseErrorTL(ErrorCode code, const Token& baseTok, Args ...  expectedTokens) -> nonstd::unexpected_type<ParseError>
{
...
StatementsParser::ParseResult StatementsParser::ParseSet(LexScanner& lexer, StatementInfoList& statementsInfo, const Token& stmtTok)
{
    ...
    // RKNN3: support 'set ns.attr = ...' (namespace member assignment)
    std::string member;
    if (vars.size() == 1 && lexer.PeekNextToken() == '.')
    {
        lexer.NextToken(); // consume '.'
        auto memTok = lexer.NextToken();
        if (memTok != Token::Identifier)
            return MakeParseError(ErrorCode::ExpectedIdentifier, memTok);
        member = AsString(memTok.value);
    }
    ...
    if (lexer.EatIfEqual('='))
    {
        ...
        statementsInfo.back().currentComposition->AddRenderer(
            std::make_shared<SetLineStatement>(std::move(vars), *expr, std::move(member)));
    }
