...
std::unordered_map<std::string, ExpressionFilter::FilterFactoryFn> s_filters = {
    ...
    { "items", &FilterFactory<filters::Items>::Create },
    ...
    { "safe", &FilterFactory<filters::SafeFilter>::Create },
    { "selectattr", FilterFactory<filters::Tester>::MakeCreator(filters::Tester::SelectAttrMode) },
    { "string", &FilterFactory<filters::StringFilter>::Create },
...
#ifdef JINJA2CPP_CHAT_ONLY
    // 以下 filter 非 chat_template 所需
#else
...
InternalValue Sort::Filter(const InternalValue& baseVal, RenderContext& context)
{
    ...
    std::sort(values.begin(), values.end(), [&attrName, oper, compType, &context](const InternalValue& val1, const InternalValue& val2) {
...
InternalValue DictSort::Filter(const InternalValue& baseVal, RenderContext& context)
{
    ...
    if (ConvertToBool(isReverseVal))
        std::sort(tempVector.begin(), tempVector.end(), [comparator](const KeyValuePair& l, const KeyValuePair& r) { return comparator(r, l); });
    else
        std::sort(tempVector.begin(), tempVector.end(), [comparator](const KeyValuePair& l, const KeyValuePair& r) { return comparator(l, r); });
...
Items::Items(FilterParams) {}

InternalValue Items::Filter(const InternalValue& baseVal, RenderContext&)
{
    const MapAdapter* map = GetIf<MapAdapter>(&baseVal);
    if (map == nullptr)
        return InternalValue();
    ...
    {
        auto val = map->GetValueByName(key);
        resultList.push_back(InternalValue(KeyValuePair{key, val}));
    }
    return InternalValue(ListAdapter::CreateAdapter(std::move(resultList)));
}

SafeFilter::SafeFilter(FilterParams) {}

InternalValue SafeFilter::Filter(const InternalValue& baseVal, RenderContext&)
{
    return baseVal;
}

StringFilter::StringFilter(FilterParams) {}

InternalValue StringFilter::Filter(const InternalValue& baseVal, RenderContext&)
{
    return TargetString(AsString(baseVal));
}
...
InternalValue GroupBy::Filter(const InternalValue& baseVal, RenderContext& context)
{
    ...
    auto equalComparator = [](const InternalValue& val1, const InternalValue& val2) {
    ...
    for (auto& item : list)
    {
        ...
        auto p = std::find_if(groups.begin(), groups.end(), [&equalComparator, &attr](const GroupInfo& i) { return equalComparator(i.grouper, attr); });
        if (p == groups.end())
...
FilterParams Map::MakeParams(FilterParams params)
{
    ...
    result.kwParams["filter"] = std::make_shared<ConstantExpression>(std::string("attr"));
...
InternalValue Map::Filter(const InternalValue& baseVal, RenderContext& context)
{
    ...
    std::transform(list.begin(), list.end(), std::back_inserter(resultList), [filter, &context](const InternalValue& val) { return filter->Filter(val, context); });
...
InternalValue SequenceAccessor::Filter(const InternalValue& baseVal, RenderContext& context)
{
    ...
    // RKNN3: length 用非 strict 转列表取 size（Python 语义：dict→key 数、string→字符数）。
    // strict ConvertToList 会把 Map/string 判为转换失败返回空。
    if (m_mode == LengthMode)
    {
        bool lenConverted = false;
        auto lenList = ConvertToList(baseVal, lenConverted, false);
        if (lenConverted)
            return static_cast<int64_t>(lenList.GetSize().value_or(0));
        return result;
    }
    ...
    auto lessComparator = [&attrName, &compType, &context](const InternalValue& val1, const InternalValue& val2) {
        InternalValue cmpRes;
    ...
    switch (m_mode)
    {
        ...
        case UniqueItemsMode:
        {
            ...
            std::stable_sort(items.begin(), items.end(), [&compType](const Item& i1, const Item& i2) {
                auto cmpRes = Apply2<visitors::BinaryMathOperation>(i1.val, i2.val, BinaryExpression::LogicalLt, compType);

                return ConvertToBool(cmpRes);
            });

            auto end = std::unique(items.begin(), items.end(), [&compType](const Item& i1, const Item& i2) {
            ...
            std::stable_sort(items.begin(), items.end(), [](const Item& i1, const Item& i2) { return i1.idx < i2.idx; });
...
{
    if (m_mode == BatchMode)
        ParseParams({ { std::string("linecount"), true }, { std::string("fill_with"), false } }, params);
    else
        ParseParams({ { std::string("slices"), true }, { std::string("fill_with"), false } }, params);
}
...
InternalValue Tester::Filter(const InternalValue& baseVal, RenderContext& context)
{
    ...
    InternalValueList resultList;
    resultList.reserve(list.GetSize().value_or(0));
    std::copy_if(list.begin(), list.end(), std::back_inserter(resultList), [this, tester, attrName, &context](const InternalValue& val) {
    ...
    : m_mode(mode)
{
    ...
    {
        case ToFloatMode:
            ParseParams({ { std::string("default"), false } }, params);
            break;
        case ToIntMode:
            ParseParams({ { std::string("default"), false }, { std::string("base"), false, static_cast<int64_t>(10) } }, params);
            break;
        case ToListMode:
        case AbsMode:
            break;
        case RoundMode:
            ParseParams({ { std::string("precision"), false }, { std::string("method"), false, std::string("common") } }, params);
            break;
    }
}
...
struct ValueConverterImpl : visitors::BaseVisitor<>
{
    ...
    InternalValue operator()(const std::basic_string<CharT>& val) const
    {
        ...
        switch (m_params.mode)
        {
            ...
            {
                auto str = val;
                result = ListAdapter::CreateAdapter(val.size(), [str](size_t idx) { return InternalValue(TargetString(str.substr(idx, 1))); });
                break;
            }
            default:
                break;
    ...
    InternalValue operator()(const nonstd::basic_string_view<CharT>& val) const
    {
        ...
        switch (m_params.mode)
        {
            ...
            {
                auto str = val;
                result = ListAdapter::CreateAdapter(val.size(), [str](size_t idx) { return InternalValue(str.substr(idx, 1)); });
                break;
            }
            default:
                break;
    ...
    InternalValue operator()(const MapAdapter& val) const
    {
        ...
        auto values = std::move(keys);
        return ListAdapter::CreateAdapter(num_keys, [values](size_t idx) { return InternalValue(values[idx]); });
    }
