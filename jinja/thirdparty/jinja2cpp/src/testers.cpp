...
std::unordered_map<std::string, IsExpression::TesterFactoryFn> s_testers = {
    ...
    // 以下 tester 非 chat_template 所需：startsWith/even/odd/lower/upper
#else
...
namespace
{
struct CaseLowerTest
{
    template<typename CharT>
    bool operator()(nonstd::basic_string_view<CharT> str) const
    {
        bool result = true;
        for (auto& ch : str)
        {
...
{
    InternalValue eqVal;
    explicit SubstringTest(InternalValue eq) : eqVal(std::move(eq)) {}
    template<typename CharT>
    bool operator()(nonstd::basic_string_view<CharT> srcStr) const
    {
        typename std::decay<decltype(srcStr)>::type emptyStrView;
        using CharT2 = typename decltype(emptyStrView)::value_type;
        std::basic_string<CharT2> emptyStr;
        auto substring = sv_to_string(srcStr);
        auto seq = GetAsSameString(srcStr, eqVal).value_or(emptyStr);
...
bool ValueTester::Test(const InternalValue& baseVal, RenderContext& context)
{
    ...
    case IsInMode:
    {
        ...
        auto seq = GetArgumentValue("eq", context);
        ...
        if (seqKind == ValueKind::List) {
            ...
            auto equalComparator = [&baseVal](const InternalValue& val) {
            ...
            InternalValue eqVal = GetArgumentValue("eq", context);
            result = ApplyStringConverter(baseVal, SubstringTest(std::move(eqVal)));
        }
        break;
    }
    case IsEvenMode:
    ...
    case IsLowerMode:
        ...
        {
            result = ApplyStringConverter(baseVal, CaseLowerTest());
        }
        break;
    case IsUpperMode:
        if (valKind != ValueKind::String)
        ...
        {
            result = ApplyStringConverter(baseVal, CaseUpperTest());
        }
        break;
