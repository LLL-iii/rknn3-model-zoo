...
template<typename T>
struct ArgPromoter<T, typename std::enable_if<std::is_fundamental<T>::value>>
{
    ArgPromoter(const T* val)
        : m_ptr(val)
    {
    ...
    template<typename U = T, typename = typename std::enable_if<std::is_convertible<T, U>::value>>
    operator U() const
    {
...
struct ArgPromoter<std::basic_string<CharT>, void>
{
    ...
    using other_string = typename std::conditional<std::is_same<CharT, char>::value, std::wstring, std::string>;
    using other_string_view = typename std::conditional<std::is_same<CharT, char>::value, nonstd::wstring_view, nonstd::string_view>;
...
{
    return ArgPromoter<typename std::decay<Arg>::type>(&arg);
}
...
struct UCInvoker
{
    ...
    template<typename ... Args>
    auto operator()(Args&& ... args) const -> typename std::enable_if<CanBeCalled<FuncTester<Args...>::type>::value, Value>
    {
        return Value(fn(Promote(args)...));
    }

    template<typename... Args>
    auto operator()(Args&&...) const -> typename std::enable_if<!CanBeCalled<FuncTester<Args...>::type>::value, Value>
    {
        return Value();
    }
};
...
struct TypedParamUnwrapper
{
    ...
    template<typename U>
    auto operator()(const U& v) -> typename std::enable_if<PromoteTester<U>::value && !std::is_same<typename std::decay<U>::type, ValueType>::value>
    {
        param->SetValue(Promote(v));
    }

    template<typename U>
    auto operator()(const U&) -> typename std::enable_if<!PromoteTester<U>::value && !std::is_same<typename std::decay<U>::type, ValueType>::value>
    {
    }
};
...
{
    auto typed_params = std::make_tuple(TypedUnwrapParam<typename typename std::decay<ArgDescr>::type::type>(GetParamValue(params, ad).data())...);
#if !optional_CPP17_OR_GREATER
