...
public:
    ...
    static auto GetAccessors() -> decltype(TypeReflection<T>::GetAccessors()) { return TypeReflection<T>::GetAccessors(); }
...
template<typename T>
using IsReflectedType = typename std::enable_if<TypeReflection<T>::value>;
...
struct ContainerReflector
{
    ...
    {
        auto accessor = ValueItemAccessor<T>(std::forward<T>(cont));
        return GenericList([accessor]() {return &accessor;});
    }

    template<typename T>
    static Value CreateFromPtr(const T* cont)
    {
        auto accessor = PtrItemAccessor<T>(cont);
        return GenericList([accessor]() {return &accessor;});
    }
    ...
    {
        auto ptr = std::move(cont);
        auto accessor = PtrItemAccessor<T>(ptr.get());
        return GenericList([ptr, accessor]() {return &accessor;});
    }
};
...
{
    static auto Create(std::set<T> val)-> decltype(ContainerReflector::CreateFromValue(std::move(val)))
    {
        return ContainerReflector::CreateFromValue(std::move(val));
    }
    static auto CreateFromPtr(const std::set<T>* val)-> decltype(ContainerReflector::CreateFromPtr(val))
    {
...
{
    static auto Create(std::vector<T> val)-> decltype(ContainerReflector::CreateFromValue(std::move(val)))
    {
        return ContainerReflector::CreateFromValue(std::move(val));
    }
    template<typename U>
    static auto CreateFromPtr(U&& val)-> decltype(ContainerReflector::CreateFromPtr(std::forward<U>(val)))
    {
...
{
    static auto Create(const T& val) -> GenericMap
    {
        auto accessor = ReflectedMapImpl<T>(val);
        return GenericMap([accessor]() {return &accessor;});
    }

    static auto CreateFromPtr(const T* val) -> GenericMap
    {
        auto accessor = ReflectedMapImpl<T>(static_cast<const T*>(val));
        return GenericMap([accessor]() {return &accessor;});
    }

    static auto CreateFromPtr(std::shared_ptr<T> val) -> GenericMap
    {
        auto ptr = val;
        auto accessor = ReflectedMapImpl<T>(ptr.get());
        return GenericMap([ptr, accessor]() {return &accessor;});
    }
};
...
{
    static auto Create(const T& val)-> decltype(Reflector<T>::CreateFromPtr(&val))
    {
...
{
    static auto Create(const T*const& val)-> decltype(Reflector<T>::CreateFromPtr(val))
    {
        return Reflector<T>::CreateFromPtr(val);
    }
...
{
    static auto Create(const std::shared_ptr<T>& val)-> decltype(Reflector<T>::CreateFromPtr(val.get()))
    {
...
struct Reflector<T&>
{
    static auto Create(T& val)-> decltype(Reflector<T>::Create(val))
    {
...
struct Reflector<const T*>
{
    ...
    static auto CreateFromPtr(const T* val)-> decltype(Reflector<T>::CreateFromPtr(val))
    {
        return Reflector<T>::CreateFromPtr(val);
    }
};
...
struct Reflector<T*>
{
    static auto Create(T* val)-> decltype(Reflector<T>::CreateFromPtr(val))
    {
        return Reflector<T>::CreateFromPtr(val);
    }
};
...
{
    static auto Create(std::shared_ptr<T> val)-> decltype(Reflector<T>::CreateFromPtr(val))
    {
        return Reflector<T>::CreateFromPtr(val);
    }
};
...
{
    static auto Create(std::basic_string<CharT> str) -> decltype(Value(std::move(str))) { return Value(std::move(str)); }
    static auto CreateFromPtr(const std::basic_string<CharT>* str) -> decltype(Value(*str)) { return Value(*str); }
};

template<typename CharT>
struct Reflector<nonstd::basic_string_view<CharT>>
{
    static auto Create(nonstd::basic_string_view<CharT> str) -> decltype(Value(std::move(str))) { return Value(std::move(str)); }
    static auto CreateFromPtr(const nonstd::basic_string_view<CharT>* str) -> decltype(Value(*str)) { return Value(*str); }
};

template<>
struct Reflector<bool>
{
    static auto Create(bool val)-> decltype(Value(val))
    {
        return Value(val);
    }
    static auto CreateFromPtr(const bool* val)-> decltype(Value(*val))
    {
        return Value(*val);
    }
};
...
{
    static auto Create(double val) -> decltype(Value(val)) { return Value(val); }
    static auto CreateFromPtr(const float* val) -> decltype(Value(static_cast<double>(*val))) { return Value(static_cast<double>(*val)); }
};
...
{
    static auto Create(double val) -> decltype(Value(val)) { return Value(val); }
    static auto CreateFromPtr(const double* val) -> decltype(Value(*val)) { return Value(*val); }
};
...
{ \
    static auto Create(Type val) -> Value \
    { \
        return Value(static_cast<int64_t>(val)); \
    } \
    static auto CreateFromPtr(const Type* val) -> Value \
    { \
        return Value(static_cast<int64_t>(*val)); \
    } \
}
