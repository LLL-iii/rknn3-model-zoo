...
using TemplateEnvResult = typename std::conditional<
    std::is_same<CharT, char>::value,
    nonstd::expected<Template, ErrorInfo>,
    nonstd::expected<TemplateW, ErrorInfoW>>::type;
...
struct Settings
{
    ...
    //! RKNN3: default true to match Python/transformers apply_chat_template environment
    bool trimBlocks = true;
    //! Enables blocks stripping (from the left) the same way as it does python Jinja2 engine
    //! RKNN3: default true to match Python/transformers apply_chat_template environment
    bool lstripBlocks = true;
...
public:
    ...
    {
        m_filesystemHandlers.push_back(FsHandler{std::move(prefix), std::shared_ptr<IFilesystemHandler>(&h, [](const IFilesystemHandler*) {})});
    }
    /*!
...
private:
    ...
    auto LoadTemplateImpl(TemplateEnv* env, std::string fileName, const T& filesystemHandlers, Cache& cache) -> TemplateEnvResult<CharT>;
