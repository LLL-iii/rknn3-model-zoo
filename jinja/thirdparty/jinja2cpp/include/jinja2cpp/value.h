...
public:
     ...
     */
    auto GetAccessor() const -> const IMapItemAccessor*
    {
        return m_accessor();
    }

    auto operator[](const std::string& name) const -> Value;
...
public:
    ...
    struct AnyOf<T, H, L...> : public std::integral_constant<bool, std::is_same<typename std::decay<T>::type, H>::value || AnyOf<T, L...>::value> {};
     ...
     */
    auto asWString() const -> const std::wstring&
    {
        return nonstd::get<std::wstring>(m_data);
    }
     ...
     */
    auto asList() const -> const ValuesList&
    {
        return *nonstd::get<RecWrapper<ValuesList>>(m_data);
    }
     ...
     */
    auto asMap() const -> const ValuesMap&
    {
        return *nonstd::get<RecWrapper<ValuesMap>>(m_data);
    }
 ...
 *
 *      return str1.asWString() + L" std::string(" + str2.asWString();
 *  };
...
inline auto GenericMap::operator[](const std::string& name) const -> Value
{
