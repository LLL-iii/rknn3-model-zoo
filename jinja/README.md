...
基于 [Jinja2Cpp](https://github.com/jinja2cpp/Jinja2Cpp)（Apache-2.0 许可证）的独立 C++ chat_template 渲染引擎，用于 RKNN 板端（RK3588 / RK3576）运行时渲染对话提示词。原始工程最低要求 C++14，本仓库已完成 **C++14 → C++11 降级改造**，可兼容更老的工具链（GCC 7.4 / NDK Clang 12 / Cygwin GCC 14）编译并部署到嵌入式板端。渲染精度与 Python `transformers.apply_chat_template` **逐字节一致**。
...
- 端到端精度 **540/540** 用例（18 模型 × 19 场景）与 Python golden 逐字节一致
...
# Linux aarch64（Linaro GCC 7.4 交叉编译）
bash build.sh -s linux -a aarch64

# Android arm64-v8a（NDK r23c Clang 12）
bash build.sh -s android -a arm64-v8a

# Windows 本机验证（Cygwin，GCC 14 / C++11）
...
### 1.5 Android 编译（RK3576）

设置 NDK 路径：

```bash
# env_android.sh 中默认工具链路径：
...
| `"..."s` 字面量 | `std::string("...")`（ `string_literals_compat.h` 已空为 no-op ） | 多处 |
...
| linux_x86（GCC 13） | **~9.1 MB** |
| linux_aarch64（GCC 7.4） | **~10.0 MB** |
| android_arm64-v8a（Clang 12） | **~11.8 MB** |
...
- 体积大头是完整引擎代码 + 链接必需的符号/重定位（解包 .o 共 ~8.2MB，.a 归档再加 ~1.2MB 索引）。aarch64/android 比 x86 大因旧工具链代码密度低 / libc++ 模板膨胀。**进一步减体积需深度功能裁剪，维护成本高**（详见 §9.1）。
...
模型来自 `<repo>/data/models/rknn3_zoo_test_*/`（21 个目录，其中 18 个含 chat_template，3 个无 chat_template 自动跳过）：
...
| 项目 | 数值 |
|:-----|:-----|
| 用例数 | **540/540 PASS**（0 FAIL） |
| 覆盖 | **18 模型 × 19 场景**（每场景 1~3 变体） |
...
| 平台 | 精度 |
|:-----|:---:|
| Linux x86_64 | **540/540 PASS** |
| Linux aarch64（RK3588） | **540/540 PASS** |
| Android arm64-v8a（RK3576） | **540/540 PASS** |
| Windows x86_64 (Cygwin) | **540/540 PASS** |
...
| function-calling | tools（单次调用闭环）/ multi_tool（连续多次调用） |
| 边界与健壮性 | system_dynamic（动态/缺失/空）/ system_only / assistant_end_missing / role_alternation（非标准顺序） |
| 内容 | special_chars（换行/引号/反斜杠/Unicode/HTML）/ injection（用户输入含 `{{` `{%`）/ template_vars（额外变量）/ multimodal（图像占位符） |
...
测试条件：540 用例（18 模型 × 19 场景）逐字节对比，与 Python golden 一致。延迟 = render_driver `--bench` 平均耗时；内存 = 板端脚本 `/proc VmRSS` 采样峰值。

| 平台 | 编译器 / 工具链 | 执行方式 | 精度 | 渲染耗时(ms) min/max/avg | 峰值内存(MB) min/max/avg |
|:-----|:-----|:-----|:---:|:---:|:---:|
| Linux x86_64 | GCC 13，C++11 | local模拟 | 540/540 | 0.004 / 5.051 / **0.167** | 13.46 / 14.03 / **13.86** |
| Linux aarch64（RK3588） | Linaro GCC 7.4，C++11 | adb 板端 | 540/540 | 0.009 / 11.178 / **0.413** | 3.43 / 5.70 / **3.83** |
| Android arm64-v8a（RK3576） | NDK r23c Clang 12，C++11 | adb 板端 | 540/540 | 0.019 / 40.881 / **1.365** | 4.01 / 6.30 / **4.38** |
| Windows x86_64 (Cygwin) | GCC 14,C++11 | local模拟 | 540/540 | 0.007 / 7.544 / **0.158** | 5.83 / 8.75 / **6.16** |

> 三平台均 **540/540 与 Python golden 逐字节一致**。
...
> 目标：渲染 <1ms、内存 <20MB —— 板端 aarch64/android 渲染 avg ~1.3ms（含进程冷启动，实际渲染在 1ms 内），内存远低于目标。
...
| 指标 | 值 |
|------|-----|
| 静态库体积 | linux_x86 **~9.1 MB** / linux_aarch64 **~10.0 MB** / android_arm64-v8a **~11.8 MB** |
...
| 项 | 大小 | 说明 |
|------|------|------|
| 机器码（`.text` 等） | ~5.3 MB（纯内容下限） | 完整 Jinja2Cpp 引擎（lexer/parser/evaluator/filters/testers） |
| 符号表 + 重定位 + 归档元数据 | ~3.8 MB | 链接所需，`strip --strip-unneeded` 已尽量压缩 |
| 三平台差异 | aarch64 比 x86 大 ~0.9MB、android 比 x86 大 ~2.7MB | 旧工具链代码密度低 / libc++ 模板膨胀 |

> **体积说明**：静态库体积大头是完整 Jinja2Cpp 引擎的代码 + 链接必需的符号/重定位信息（解包后 .o 共 ~8.2MB，.a 归档再占 ~1.2MB 索引）。已做的裁剪：`JINJA2CPP_CHAT_ONLY`（去掉非 chat_template 的 filter/测试器/语句）、`-Os -DNDEBUG`、merge 时 `strip --strip-unneeded`。进一步减体积需深度功能裁剪或换更轻引擎，维护成本高。最终板端链接时用 `-Wl,--gc-sections` 可裁掉未用代码（render_driver 已启用，strip 后 ~1.1MB）。
...
# 2) 本机模拟（install 传绝对路径；Linux 下直接执行，Windows 需设置 CYGWIN_BIN 才走 cygwin 包装）
...
# → PASS 540/540 即与 Python golden 逐字节一致
...
A: 体积大头是**完整 Jinja2Cpp 引擎的代码**（lexer/parser/evaluator/filters，纯内容 ~5.3 MB）+ **链接必需的符号表/重定位/归档索引**（~3.8 MB）。解包后 .o 共 ~8.2 MB，.a 归档再加 ~1.2 MB 索引。`JINJA2CPP_CHAT_ONLY` 裁剪、`-Os`、merge 时 `strip --strip-unneeded` 均已启用。若要 <5 MB 需深度功能裁剪或换更轻引擎（维护成本高）。
...
A: 为兼容更老工具链（Linaro GCC 7.4 / NDK r23c / 嵌入式板端）。Jinja2Cpp 原始要求 C++14，本仓库已完成 C++11 改造（详见 §2），540/540 测试验证功能无损。
