...
const char* system_prompt  = "<|im_start|>System: You are a useful assistant for concise replies.<end_of_utterance>\n";
const char* prompt_prefix  = "User: ";
const char* prompt_postfix = "<end_of_utterance>\nAssistant:";
...
int main(int argc, char **argv)
{
    ...
    ret = init_smol_vl_model(&rknn_app_ctx, llm_model_path, llm_weight_path, vision_model_path, vision_weight_path, &params, n_params, callback, vision_core_mask, llm_core_mask);
    if (ret != 0)
    {
        printf("init_smol_vl_model fail! ret=%d llm_model_path=%s vision_model_path=%s\n", ret, llm_model_path, vision_model_path);
        goto out;
    }
    ...
    tensor.image.image_start      = "<fake_token_around_image><global-img>";
    tensor.image.image_end        = "<fake_token_around_image>";
    tensor.image.image_content    = "<image>";
    tensor.enable_thinking = false;

    ret = inference_smol_vl_model(&rknn_app_ctx, &src_image, img_embeds, tensor, n_inputs, &perf);
    if (ret != 0)
    {
        printf("inference smol_vl model fail! ret=%d\n", ret);
        goto out;
    }

    printf_perf(&perf);
out:
    ret = release_smol_vl_model(&rknn_app_ctx);

    if (ret != 0)
    {
        printf("release smol_vl model fail! ret=%d\n", ret);
    }
