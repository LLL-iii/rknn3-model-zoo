...
const rknn3_sampling_params SAMPLE_PARAMS = {
    ...
    .repeat_penalty = 1.05f,   // Please refer to generation_config.json to configure the corresponding parameters. eg, //huggingface.co/Qwen/Qwen2.5-VL-3B-Instruct/blob/main/generation_config.json
...
int main(int argc, char **argv)
{
    ...
    ret = init_qwen2_5_vl_model(&rknn_app_ctx, llm_model_path, llm_weight_path, vision_model_path, vision_weight_path, &params, n_params, callback, vision_core_mask, llm_core_mask);
    if (ret != 0)
    {
        printf("init_qwen2_5_vl_model fail! ret=%d llm_model_path=%s vision_model_path=%s\n", ret, llm_model_path, vision_model_path);
        goto out;
    }
    ...
    ret = inference_qwen2_5_vl_model(&rknn_app_ctx, &src_image, img_embeds, tensor, n_inputs, &perf, speedup_ratio);
    if (ret != 0)
    {
        printf("inference qwen2_5_vl model fail! ret=%d\n", ret);
        goto out;
    }

    printf_perf(&perf);
out:
    ret = release_qwen2_5_vl_model(&rknn_app_ctx);

    if (ret != 0)
    {
        printf("release qwen2_5_vl model fail! ret=%d\n", ret);
    }
