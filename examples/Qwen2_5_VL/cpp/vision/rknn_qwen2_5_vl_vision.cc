...
int init_qwen2_5_vl_vision(rknn_qwen2_5_vl_vision_context* vision_ctx, const char* model_path, const char* weight_path, uint32_t core_mask, uint32_t model_width, uint32_t model_height)
{
    ...
    {   
        int feature_size = MODEL_WIDTH * MODEL_HEIGHT / 14 / 14;
        if(model_width != 0 && model_height != 0) {
            feature_size = model_width * model_height / 14 / 14;
        }
...
void transpose(float16 *in, float16 *out, int model_width, int model_height) {
    int64_t dims[8] = {2,3,int(model_height/28),2,14,int(model_width/28),2,14};
