...
int main(int argc, char **argv)
{
    ...
    ret = init_internvl3_model(&rknn_app_ctx, llm_model_path, llm_weight_path, vision_model_path, vision_weight_path, &params, n_params, callback, vision_core_mask, llm_core_mask);
    if (ret != 0)
    {
        printf("init_internvl3_model fail! ret=%d llm_model_path=%s vision_model_path=%s\n", ret, llm_model_path, vision_model_path);
        goto out;
    }
    ...
    ret = inference_internvl3_model(&rknn_app_ctx, &src_image, img_embeds, tensor, n_inputs, &perf);
    if (ret != 0)
    {
        printf("inference internvl3 model fail! ret=%d\n", ret);
        goto out;
    }
...
out:
    ret = release_internvl3_model(&rknn_app_ctx);
    if (ret != 0)
    {
        printf("release internvl3 model fail! ret=%d\n", ret);
    }
