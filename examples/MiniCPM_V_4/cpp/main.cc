...
int main(int argc, char **argv)
{
    ...
    ret = init_minicpmv4_model(&rknn_app_ctx, llm_model_path, llm_weight_path, vision_model_path, vision_weight_path, &params, n_params, callback, vision_core_mask, llm_core_mask);
    if (ret != 0)
    {
        printf("init_minicpmv4_model fail! ret=%d llm_model_path=%s vision_model_path=%s\n", ret, llm_model_path, vision_model_path);
        goto out;
    }
    ...
    tensor.prompt               = prompt_new.c_str();
    ...
    tensor.image.image_content  = "<unk>";
    tensor.enable_thinking = false;

    ret = inference_minicpmv4_model(&rknn_app_ctx, &src_image, img_embeds, tensor, n_inputs, &perf);
    if (ret != 0)
    {
        printf("inference minicpmv4 model fail! ret=%d\n", ret);
        goto out;
    }
...
out:
    ret = release_minicpmv4_model(&rknn_app_ctx);
    if (ret != 0)
    {
        printf("release minicpmv4 model fail! ret=%d\n", ret);
    }
