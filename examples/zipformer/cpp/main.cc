...
int main(int argc, char **argv)
{
    if (argc != 8)
    {
        printf("%s <encoder_path> <encoder_weight_path> <decoder_path> <decoder_weight_path> <joiner_path> <joiner_weight_path> <audio_path>\n", argv[0]);
        return -1;
    }

    const char *encoder_path = argv[1];
    const char *encoder_weight_path = argv[2];
    const char *decoder_path = argv[3];
    const char *decoder_weight_path = argv[4];
    const char *joiner_path = argv[5];
    const char *joiner_weight_path = argv[6];
    ...
    if (!file_exists(encoder_path)) {
        printf("Error: Encoder model file not found: %s\n", encoder_path);
        return -1;
    }
    if (!file_exists(encoder_weight_path)) {
        printf("Error: Encoder weight file not found: %s\n", encoder_weight_path);
        return -1;
    }
    if (!file_exists(decoder_path)) {
        printf("Error: Decoder model file not found: %s\n", decoder_path);
        return -1;
    }
    if (!file_exists(decoder_weight_path)) {
        printf("Error: Decoder weight file not found: %s\n", decoder_weight_path);
        return -1;
    }
    if (!file_exists(joiner_path)) {
        printf("Error: Joiner model file not found: %s\n", joiner_path);
        return -1;
    }
    if (!file_exists(joiner_weight_path)) {
        printf("Error: Joiner weight file not found: %s\n", joiner_weight_path);
        return -1;
    }
    ...
    float frame_shift_s = (static_cast<float>(frame_shift_ms) / 1000.0f) * subsampling_factor;
    ...
    timer.tik();
    ret = init_zipformer_model(encoder_path, encoder_weight_path, &rknn_app_ctx.encoder_context, 0x1);
    if (ret != 0)
    {
    ...
    timer.tik();
    ret = init_zipformer_model(decoder_path, decoder_weight_path, &rknn_app_ctx.decoder_context, 0x1);
    if (ret != 0)
    {
    ...
    timer.tik();
    ret = init_zipformer_model(joiner_path, joiner_weight_path, &rknn_app_ctx.joiner_context, 0x1);
    if (ret != 0)
    {
