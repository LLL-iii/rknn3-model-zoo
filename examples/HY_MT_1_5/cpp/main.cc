...
const char* prompt_prefix  = "<｜hy_begin▁of▁sentence｜><｜hy_User｜>";
const char* prompt_postfix = "<｜hy_place▁holder▁no▁8｜>";
...
int main(int argc, char **argv)
{
    if (argc != 7)
    {
        printf("%s <model_path> <weight_path> <tokenizer_path> <embedding_path> <core_mask> <prompt>\nFor example:\n./rknn_hunyuan_demo HY-MT1.5-1.8B.rknn HY-MT1.5-1.8B.weight HY-MT1.5-1.8B.tokenizer.gguf HY-MT1.5-1.8B.embed.bin 0xff \"Translate the following segment into Chinese, without additional explanation.\\n\\nIt’s on the house.\"\n", argv[0]);
        return -1;
    }
    ...
    // RKNN Context
    rknn_hunyuan_llm_context rknn_app_ctx;
    memset(&rknn_app_ctx, 0, sizeof(rknn_hunyuan_llm_context));
    ...
    // Set LLM parameters for Hunyuan
    ...
    printf("--> init hunyuan llm model\n");  
    ret = init_hunyuan_llm(&rknn_app_ctx, model_path, weight_path, &params, n_params, callback, core_mask);
    if (ret != 0)
    {
        printf("init_hunyuan_llm fail! ret=%d model_path=%s weight_path=%s\n", ret, model_path, weight_path);
        goto out;
    }
    ...
    printf("--> inference hunyuan llm model\n");  
    ret = inference_hunyuan_llm(&rknn_app_ctx, tensor, n_inputs, &perf);
    if (ret != 0)
    {
        printf("inference hunyuan llm fail! ret=%d\n", ret);
        goto out;
    }
...
out:
    ret = release_hunyuan_llm(&rknn_app_ctx);
    if (ret != 0)
    {
        printf("release hunyuan llm fail! ret=%d\n", ret);
    }
