...
int main(int argc, char **argv)
{
    if (argc != 7)
    {
        ...
        printf("  tokenizer_dir: HF 模型目录（含 tokenizer.json / tokenizer_config.json / chat_template.jinja）\n");
        return -1;
    }
    ...
    // RKNN Context
    rknn_qwen2_5_llm_context rknn_app_ctx;
    memset(&rknn_app_ctx, 0, sizeof(rknn_qwen2_5_llm_context));
    ...
    // Chat template 与渲染缓冲：提前声明，避免后续 goto out 跨过非平凡初始化
    ...
    printf("--> init qwen2_5 llm model\n");  
    ret = init_qwen2_5_llm(&rknn_app_ctx, model_path, weight_path, &params, n_params, callback, core_mask);
    if (ret != 0)
    {
        printf("init_qwen2_5_llm fail! ret=%d model_path=%s weight_path=%s\n", ret, model_path, weight_path);
        goto out;
    }
    ...
    printf("--> inference qwen2_5 llm model\n");  
    ret = inference_qwen2_5_llm(&rknn_app_ctx, tensor, n_inputs, &perf);
    if (ret != 0)
    {
        printf("inference qwen2_5 llm fail! ret=%d\n", ret);
        goto out;
    }
...
out:
    ret = release_qwen2_5_llm(&rknn_app_ctx);
    if (ret != 0)
    {
        printf("release qwen2_5 llm fail! ret=%d\n", ret);
    }
