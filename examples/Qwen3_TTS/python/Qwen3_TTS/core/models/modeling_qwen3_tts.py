...
class Qwen3TTSTalkerQuantDataStop(RuntimeError):
    """Raised when talker quant-data collection reaches the configured limit."""
...
def apply_multimodal_rotary_pos_emb(q, k, cos, sin, mrope_section, mrope_interleaved=False, unsqueeze_dim=1, direct=False):
    ...
    """
    if mrope_interleaved and not direct:
...
def _interleaved_rope(cos, sin, mrope_section, mrope_interleaved, unsqueeze_dim):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        query_states, key_states = apply_multimodal_rotary_pos_emb(
            query_states, key_states, cos, sin, self.rope_scaling["mrope_section"], self.rope_scaling["interleaved"], direct=True,
        )
...
class Qwen3TTSTalkerCodePredictorModelForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Prefill stage
        if inputs_embeds is not None and inputs_embeds.shape[1] > 1 and generation_steps is None:
        ...
        if num_logits_to_keep is None:
            logits = (hidden_states @ torch.cat([head.weight.data.T.unsqueeze(0) for head in self.lm_head], dim=0)[generation_steps,...])[:, -1:, :]
        else:
            logits = (hidden_states @ torch.cat([head.weight.data.T.unsqueeze(0) for head in self.lm_head], dim=0)[generation_steps,...])[:, num_logits_to_keep, :]
...
class Qwen3TTSTalkerModel(Qwen3TTSTalkerTextPreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # create position embeddings to be shared across the decoder layers
        position_embeddings = self.rotary_emb(hidden_states, position_ids)
        # position_embeddings = _interleaved_rope(*position_embeddings, mrope_section=self.config.rope_scaling['mrope_section'], mrope_interleaved=self.config.rope_scaling['interleaved'], unsqueeze_dim=1)
        position_embeddings = (position_embeddings[0][0:1, ...], position_embeddings[1][0:1, ...]) 
...
class Qwen3TTSTalkerForConditionalGeneration(Qwen3TTSTalkerTextPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Qwen3TTSTalkerConfig):
        ...
        self.return_last_hidden_only = True
        self.quant_data_enabled = False
        self.quant_data_save_dir = None
        self.quant_data_sample_count = 0
        self.quant_data_seen_hashes = set()
    ...
    def configure_quant_data_collection(
        self,
        enabled: bool = False,
        save_dir: Optional[str] = None,
        return_last_hidden_only: Optional[bool] = None,
        reset: bool = False,
    ) -> None:
        if return_last_hidden_only is not None:
            self.return_last_hidden_only = bool(return_last_hidden_only)

        self.quant_data_enabled = bool(enabled and save_dir)
        self.quant_data_save_dir = save_dir

        if not self.quant_data_enabled:
            return

        os.makedirs(self.quant_data_save_dir, exist_ok=True)
        if reset:
            self.quant_data_sample_count = 0
            self.quant_data_seen_hashes.clear()
            open(os.path.join(self.quant_data_save_dir, "_dataset.txt"), "w", encoding="utf-8").close()

    def _maybe_collect_quant_data(
        self,
        inputs_embeds: torch.Tensor,
        attention_mask: Optional[torch.Tensor],
        position_ids: Optional[torch.Tensor],
    ) -> None:
        if not self.quant_data_enabled or self.quant_data_save_dir is None:
            return
        ...
        if position_ids is None:
            position_ids = torch.arange(seq_len, device=device, dtype=torch.int32).unsqueeze(0).expand(batch_size, -1)

        inputs_np = inputs_embeds.detach().to(torch.float32).cpu().numpy()
        attn_np = attention_mask.detach().to(torch.float32).cpu().numpy()
        pos_np = position_ids.detach().to(torch.int32).cpu().numpy()
        num_logits_np = np.array([seq_len - 1], dtype=np.int64)

        digest = hashlib.sha256()
        for array in (inputs_np, attn_np, pos_np, num_logits_np):
            digest.update(str(array.shape).encode("utf-8"))
            digest.update(str(array.dtype).encode("utf-8"))
            digest.update(array.tobytes())
        sample_hash = digest.hexdigest()

        if sample_hash in self.quant_data_seen_hashes:
            return
        self.quant_data_seen_hashes.add(sample_hash)

        self.quant_data_sample_count += 1
        prefix = f"{self.quant_data_sample_count:06d}"
        file_inputs = f"inputs_embeds_{prefix}.npy"
        file_attn = f"attention_mask_{prefix}.npy"
        file_pos = f"position_ids_{prefix}.npy"
        file_num = f"num_logits_to_keep_{prefix}.npy"

        np.save(os.path.join(self.quant_data_save_dir, file_inputs), inputs_np)
        np.save(os.path.join(self.quant_data_save_dir, file_attn), attn_np)
        np.save(os.path.join(self.quant_data_save_dir, file_pos), pos_np)
        np.save(os.path.join(self.quant_data_save_dir, file_num), num_logits_np)

        with open(os.path.join(self.quant_data_save_dir, "_dataset.txt"), "a", encoding="utf-8") as dataset_file:
            dataset_file.write(f"{file_inputs} {file_attn} {file_pos} {file_num}\n")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is not None and position_ids is None:
            ...
            #! 如果不是在导出 onnx 步骤, 不应该跑到这里
            # print('Warning: position_ids is provided externally!')
            # if position_ids.shape[0] == 1:
            #     position_ids = position_ids.unsqueeze(0).expand(3, -1, -1)
            position_ids = position_ids.reshape(1,1,-1)
            pass

        self._maybe_collect_quant_data(
            inputs_embeds=inputs_embeds,
            attention_mask=attention_mask,
            position_ids=position_ids.reshape(position_ids.shape[0], -1) if position_ids is not None else None,
        )
        ...
        return Qwen3TTSTalkerOutputWithPast(
            ...
            attentions=outputs.attentions,
            past_hidden=hidden_states[:, -1:, :] if self.return_last_hidden_only else hidden_states,
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        prepare_quant_data_dir = kwargs.pop("prepare_quant_data_dir", None)
        prepare_quant_return_last_hidden = kwargs.pop("prepare_quant_return_last_hidden", True)
        ...
        self.talker.configure_quant_data_collection(
            enabled=prepare_quant_data_dir is not None,
            save_dir=prepare_quant_data_dir,
            return_last_hidden_only=prepare_quant_return_last_hidden if prepare_quant_data_dir is not None else False,
        )
