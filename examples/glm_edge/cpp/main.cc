...
int main(int argc, char **argv)
{
    ...
    // RKNN Context
    rknn_glm_llm_context rknn_app_ctx;
    memset(&rknn_app_ctx, 0, sizeof(rknn_glm_llm_context));
    ...
    printf("--> init glm llm model\n");
    ret = init_glm_llm(&rknn_app_ctx, model_path, weight_path, &params, n_params, callback, core_mask);
    if (ret != 0)
    {
        printf("init_glm_llm fail! ret=%d model_path=%s weight_path=%s\n", ret, model_path, weight_path);
        goto out;
    }
    ...
    printf("--> inference glm llm model\n");
    ret = inference_glm_llm(&rknn_app_ctx, tensor, n_inputs, &perf);
    if (ret != 0)
    {
        printf("inference glm llm fail! ret=%d\n", ret);
        goto out;
    }
...
out:
    ret = release_glm_llm(&rknn_app_ctx);
    if (ret != 0)
    {
        printf("release glm llm fail! ret=%d\n", ret);
    }
