...
class Gemma4ClippableLinear(nn.Module):
    ...
    def _dynamic_clamp(self, hidden_states, min_value, max_value):
        zero = hidden_states.sum() * 0.0

        min_value = min_value.to(dtype=hidden_states.dtype, device=hidden_states.device) + zero
        max_value = max_value.to(dtype=hidden_states.dtype, device=hidden_states.device) + zero

        return torch.clamp(hidden_states, min=min_value, max=max_value)

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if self.use_clipped_linears:
            hidden_states = self._dynamic_clamp(hidden_states, self.input_min, self.input_max)

        hidden_states = self.linear(hidden_states)

        if self.use_clipped_linears:
            hidden_states = self._dynamic_clamp(hidden_states, self.output_min, self.output_max)
...
class Gemma4TextDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attention_mask = attention_mask_local if self.self_attn.is_sliding else attention_mask_global
...
class Gemma4TextScaledWordEmbedding_RK(nn.Embedding):
    ...
    def fuse_scale(self):
        if not self._scale_fused and self.scalar_embed_scale != 1.0:
            with torch.no_grad():
                self.weight.mul_(self.scalar_embed_scale)
...
class Gemma4PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, Gemma4TextScaledWordEmbedding_RK):
            init.constant_(module.embed_scale, module.scalar_embed_scale)
...
class Gemma4TextModel(Gemma4PreTrainedModel):
    ...
    def __init__(self, config: Gemma4TextConfig):
        ...
        self.embed_tokens = Gemma4TextScaledWordEmbedding_RK(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        for layer_type in self.unique_layer_types:
            if layer_type == "full_attention":
                position_embeddings[layer_type] = self.rotary_emb(hidden_states, position_ids_global, layer_type)
        ...
        for i, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            hidden_states = decoder_layer(
                ...
                position_ids=position_ids,
                past_key_values=past_key_values,
                attention_mask_global=causal_mask_mapping_rknn['full_attention'],
                attention_mask_local=causal_mask_mapping_rknn['sliding_attention'],
                **kwargs,
            )
...
class Gemma4ForConditionalGeneration(Gemma4PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # slice_indices = slice(-logits_to_keep, None) if isinstance(logits_to_keep, int) else logits_to_keep
        # logits = self.lm_head(hidden_states[:, slice_indices, :])
        logits = self.lm_head(hidden_states.index_select(dim=1, index=logits_to_keep))
