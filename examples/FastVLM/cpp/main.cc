...
int main(int argc, char **argv)
{
    ...
    memcpy(params.vocab_info.special_bos_id, vocab_info.special_bos_id, RKNN3_MAX_SPECIAL_BOS_ID_NUM * sizeof(int));
    memcpy(params.vocab_info.special_eos_id, vocab_info.special_eos_id, RKNN3_MAX_SPECIAL_EOS_ID_NUM * sizeof(int));
    ...
    ret = init_fastvlm_model(&rknn_app_ctx, llm_model_path, llm_weight_path, vision_model_path, vision_weight_path, &params, n_params, callback, vision_core_mask, llm_core_mask);
    if (ret != 0)
    {
        printf("init_fastvlm_model fail! ret=%d llm_model_path=%s vision_model_path=%s\n", ret, llm_model_path, vision_model_path);
        goto out;
    }
    ...
    tensor.image.image_start = "<|im_start|>";
    tensor.image.image_end = "<|im_end|>";
    tensor.image.image_content = "<|image_pad|>";
    tensor.enable_thinking = false;

    ret = inference_fastvlm_model(&rknn_app_ctx, &src_image, img_embeds, tensor, n_inputs, &perf);
    if (ret != 0)
    {
        printf("inference fastvlm model fail! ret=%d\n", ret);
        goto out;
    }
    printf_perf(&perf);

out:
    ret = release_fastvlm_model(&rknn_app_ctx);
    if (ret != 0)
    {
        printf("release fastvlm model fail! ret=%d\n", ret);
    }
