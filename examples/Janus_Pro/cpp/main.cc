...
const char* system_prompt  = "'You are a helpful language and vision assistant. You are able to understand the visual content that the user provides, and assist the user with a variety of tasks using natural language.\n\n";
...
int main(int argc, char **argv)
{
    ...
    ret = init_janus_model(&rknn_app_ctx, llm_model_path, llm_weight_path, vision_model_path, vision_weight_path, &params, n_params, callback, vision_core_mask, llm_core_mask);
    if (ret != 0)
    {
        printf("init_janus_model fail! ret=%d llm_model_path=%s vision_model_path=%s\n", ret, llm_model_path, vision_model_path);
        goto out;
    }
    ...
    ret = inference_janus_model(&rknn_app_ctx, &src_image, img_embeds, tensor, n_inputs, &perf);
    if (ret != 0)
    {
        printf("inference janus model fail! ret=%d\n", ret);
        goto out;
    }
    printf_perf(&perf);

out:
    ret = release_janus_model(&rknn_app_ctx);
    if (ret != 0)
    {
        printf("release janus model fail! ret=%d\n", ret);
    }
