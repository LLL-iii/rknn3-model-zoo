 ...
 *
 * 自动检测：优先加载 tokenizer.json 走 HFTokenizer（BPE）；
 * 仅当目录无 tokenizer.json 时才回退 tokenizer.model（SentencePiece 后端）。
 */
...
| 18 | MiniCPM-V-4 | openbmb/MiniCPM-V-4 | BPE |
| 19 | FastVLM | llava-hf/llava-1.5-7b-hf | BPE |
...
### 8.1.1 100k 超长文本编码（单条 10 万字符）

测试：单条 100,000 字符（UTF-8 151,913 字节）混合文本。纯编码耗时 = 长文本端到端中位数 − 空文本端到端中位数（消除模型加载），3 次取中位数。**Qwen3 代表值**（33,580 tokens）：

| 平台 | 纯编码 (ms) | 峰值 RSS (MB) | 吞吐 (kchars/s) |
|:-----|--------:|--------:|---------------:|
| x86_64 | 33.5 | — | 2,985 |
| Linux aarch64（RK3588） | 60.0 | 28.0 | 1,665 |
| Android arm64-v8a（RK3576） | 225.5 | 25.9 | 443 |

**分模型纯编码耗时**（毫秒，已启用 PCRE2 JIT，21 个模型全覆盖）：

| 模型 | x86_64 | RK3588 | RK3576 |
|:-----|------:|------:|------:|
| Qwen3 | 33.5 | 60.0 | 225.5 |
| Qwen2_5 | 37.2 | 74.2 | 235.4 |
| Qwen2_5_VL | 37.0 | 67.1 | 50.4 |
| Qwen2_5_Omni | 31.7 | 80.7 | 344.0 |
| Qwen3_VL | 53.5 | 71.2 | 259.9 |
| Qwen3_ASR | 43.7 | 39.3 | 305.2 |
| Qwen3_Embedding | 35.3 | 86.1 | 215.7 |
| Qwen3_Reranker | 24.5 | 60.2 | 222.1 |
| Qwen3_TTS | 51.4 | 73.1 | 256.3 |
| Qwen3_5 | 44.7 | 52.1 | 271.9 |
| Qwen3_VL_LoRA | 37.5 | 45.4 | 335.4 |
| GME-Qwen2-VL | 34.8 | 70.0 | 320.4 |
| InternVLM | 34.6 | 49.9 | 165.5 |
| HY_MT_1_5 | 35.1 | 50.4 | 111.4 |
| Janus_Pro | 41.9 | 101.6 | 309.8 |
| SmolVLM | 21.6 | 38.5 | 106.0 |
| SmolVLM2 | 27.0 | 40.8 | 140.7 |
| glm_edge | 40.3 | 69.1 | 220.1 |
| MiniCPM_V_4 | 28.4 | 85.8 | 277.2 |
| FastVLM | 27.0 | 76.6 | 274.2 |
| gemma4 | 57.8 | 110.2 | 400.2 |
| **平均** | **37.1** | **66.8** | **240.3** |

> **JIT 优化**：瓶颈为 Sequence `[Split, ByteLevel]` 预分词的正则匹配（PCRE2 每 match 固定开销 ~50-100µs，混合文本切 4 万+ piece）。通过 `PCRE2_SUPPORT_JIT` 强制开启 + `pcre2_jit_match`，单条 100k 编码从 0.74/2.19/3.82 s 降到 **37.1 / 66.8 / 240.3 ms**（-95%），Qwen3 提速 27~50 倍，达到/超过 llama.cpp 水平。Cygwin 因 sljit 运行时 segfault 禁用 JIT，Linux/Android 启用。
>
> **FastVLM / MiniCPM-V-4 专项优化**（8_26 重测）：FastVLM 目录同时含 `tokenizer.model`(SPM) 与 `tokenizer.json`(BPE)，两者对同文本**逐 token 等价**，原实现 SPM 优先；改为 tokenizer.json(BPE) 优先、SPM 作 fallback 后，RK3588 纯编码 117.5→**76.6 ms**（-35%），x86 27.0 ms。MiniCPM-V-4 的 `HFWord::merge_all` 将候选结构压到 12B（uint32 rank/pos/version）、初始构建改用 `std::make_heap`（O(n) 替代逐 push O(n log n)）、`alive` 用 `vector<uint8_t>`，cygwin 本地 26→18.6 ms。两模型 x86 端 100k 编码均进入 27~28 ms，RK3588 均低于 86 ms。

### 8.1.2 三版本数据集耗时对比

40,000 条文本数据集（EN 20,000 + ZH 20,000）C++ 编码单文本平均耗时（ms/text）。

| 模型 | 本仓库 (aarch64) | llama.cpp (aarch64) | meta-pytorch (x86_64) |
|:-----|------:|------:|------:|
| Qwen3 | 0.26 | 0.09 | 0.15 |
| Qwen2_5 | 0.26 | 0.09 | 0.16 |
| Qwen2_5_VL | 0.26 | 0.09 | 0.16 |
| Qwen2_5_Omni | 0.26 | 0.09 | 0.16 |
| Qwen3_VL | 0.26 | 0.09 | 0.16 |
| Qwen3_ASR | 0.26 | — | 0.16 |
| Qwen3_Embedding | 0.26 | 0.09 | 0.16 |
| Qwen3_Reranker | 0.26 | 0.09 | 0.16 |
| Qwen3_TTS | 0.26 | — | 0.16 |
| HY_MT_1_5 | 0.26 | 0.12 | 0.00* |
| Janus_Pro | 0.34 | 0.20 | 0.00* |
| SmolVLM | 0.27 | 0.06 | 0.12 |
| SmolVLM2 | 0.27 | 0.06 | 0.12 |
| glm_edge | 0.25 | 0.08 | 0.17 |
| GME-Qwen2-VL | 0.26 | 0.10 | 0.16 |
| InternVLM | 0.26 | 0.09 | 0.16 |
| MiniCPM_V_4 | 0.14 | 0.09 | 0.06 |
| FastVLM | 0.24 | 0.07 | 0.05 |
| gemma4 | 0.20 | 0.12 | — |

> 注：本仓库为 RK3588 板端 `--stdin-batch` 端到端均摊（含 demo 启动 + 模型加载，加载被 40K 文本摊薄）；llama.cpp 为 RK3588 板端（compare.py，GGUF 加载）；meta-pytorch 为 x86_64 PC（上游原版 C++20）。**平台与测量口径不同，仅供参考**。`*`：meta-pytorch 对 HY_MT_1_5 / Janus_Pro 编码未完全支持，耗时异常偏小。三版本共同模型（16 个）中，本仓库 RK3588 端到端均摊耗时约 0.14~0.34 ms/text，llama.cpp 约 0.06~0.21 ms/text，meta-pytorch x86 约 0.05~0.17 ms/text。
...
> - **原 SPM 模型**（MiniCPM_V_4、FastVLM）：两模型现已改走 BPE 后端（见 8.1.1），由 tokenizer.json 加载。上表为历史三路对比：改造后本仓库与 llama.cpp 原方案基本持平（MiniCPM 15.1 vs 15.5、FastVLM 10.1 vs 7.9），但仍远低于 meta-pytorch 上游原版（54.9 → 15.1、31.4 → 10.1 MB）。换来的是零 GGUF 格式转换、直接加载 tokenizer.json、精度 100%。
