...
struct llama_model_loader {
    ...
    struct llama_tensor_weight {
        ...
        //     if (tensor_idx < 0) {
        //         throw_runtime_error(format("tensor '%s' not found in the model", ggml_get_name(tensor)));
        //     }

        //     offs = gguf_get_data_offset(gguf_ctx) + gguf_get_tensor_offset(gguf_ctx, tensor_idx);
        //     if (offs + ggml_nbytes(tensor) < offs || offs + ggml_nbytes(tensor) > file->size()) {
        //         throw_runtime_error(format("tensor '%s' data is not within the file bounds, model is corrupted or incomplete", ggml_get_name(tensor)));
        //     }
        }
    };
