...
struct gguf_reader {
    ...
    bool read(std::string & dst) const {
        ...
        dst.resize(size);
        return fread((void*)(dst.data()), 1, dst.length(), file) == dst.length();
    }
...
struct gguf_buff_reader {
    const char * buffer;
    size_t buffer_size;
    mutable size_t buffer_pos;

    gguf_buff_reader(const void * buf, size_t size)
        : buffer(reinterpret_cast<const char *>(buf)), buffer_size(size), buffer_pos(0) {}

    template <typename T>
    bool read(T & dst) const {
        if (buffer_pos + sizeof(T) > buffer_size) return false;
    ...
    bool read(std::string & dst) const {
        ...
        if (buffer_pos + size > buffer_size) return false;
        dst.resize(size);
        memcpy((void *)dst.data(), buffer + buffer_pos, size);
        buffer_pos += size;
        return true;
    }
...
template<typename T>
bool gguf_read_emplace_helper(const struct gguf_buff_reader & gr, std::vector<struct gguf_kv> & kv, const std::string & key, const bool is_array, const size_t n) {
    if (is_array) {
        std::vector<T> value;
        try {
            if (!gr.read(value, n)) {
...
struct gguf_context * gguf_init_from_file_impl(FILE * file, struct gguf_init_params params) {
    ...
    if (ok && gr.read(ctx->version)) {
        if (ok && ctx->version == 0) {
            fprintf(stderr, "%s: bad GGUF version: %" PRIu32 "\n", __func__, ctx->version);
            ok = false;
        }
        ...
        */
        if (ok && (ctx->version & 0x0000FFFF) == 0x00000000) {
            fprintf(stderr, "%s: failed to load model: this GGUF file version %" PRIu32 " is extremely large, is there a mismatch between the host and model endianness?\n", __func__, ctx->version);
            ok = false;
        }
    ...
    //             fprintf(stderr,"%s: tensor '%s' size overflow, cannot accumulate size %zu + %zu\n",
...
struct gguf_context * gguf_init_from_buffer_impl(const void *buffer, size_t buffer_size, struct gguf_init_params params) {
    const struct gguf_buff_reader gr(buffer, buffer_size);
    struct gguf_context * ctx = new gguf_context;
    ...
    // we require the data section to be aligned, so take into account any padding
    size_t aligned_pos = GGML_PAD(gr.buffer_pos, ctx->alignment);
    if (aligned_pos > gr.buffer_size) {
        printf("%s Warning: internal aligned_pos=%zu alignment=%zu buffer_pos=%zu\n", __func__, aligned_pos, ctx->alignment, gr.buffer_pos);
    }
    gr.buffer_pos = aligned_pos;
    ctx->offset = gr.buffer_pos;
    ...
    //         // read the binary blob with the tensor data
    //         if (gr.buffer_pos + ctx->size > gr.buffer_size) {
    //             fprintf(stderr, "%s: failed to read tensor data binary blob (buffer overflow)\n", __func__);
    //             ggml_free(ctx_data);
    //             *params.ctx = nullptr;
    //             gguf_free(ctx);
    //             return nullptr;
    //         }
    //         memcpy(data->data, gr.buffer + gr.buffer_pos, ctx->size);
    //         gr.buffer_pos += ctx->size;
...
struct gguf_context * gguf_init_from_buffer(const void * buffer, size_t buffer_size, struct gguf_init_params params) {
    struct gguf_context * result = gguf_init_from_buffer_impl(buffer, buffer_size, params);
    return result;
}
...
//         fprintf(stderr,"%s: failed to write GGUF data into '%s': %s\n", __func__, fname, ex.what());
//         fclose(file);
//         return false;
//     }
