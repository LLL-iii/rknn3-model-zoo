...
int main(int raw_argc, char ** raw_argv) {
    ...
    llama_backend_init();
    llama_vocab * mVocab = llama_load_vocab_from_file(model_path, true, false, NULL);

    const bool model_wants_add_bos = llama_vocab_get_add_bos(mVocab);
    ...
    tokens = common_tokenize(mVocab, prompt, add_bos, parse_special);
    ...
    for (int i = 0; i < (int) tokens.size(); i++) {
        if (printing_ids) {
            ...
            printf("%6d -> '", tokens[i]);
            write_utf8_cstr_to_stdout(common_token_to_piece(mVocab, tokens[i]).c_str(), invalid_utf8);
            if (invalid_utf8) {
